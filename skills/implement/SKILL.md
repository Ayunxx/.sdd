---
description: 实现（编排器）/ Orchestrate task implementation wave-by-wave — spawn an isolated implementer subagent per task (parallel within a wave). 上下文隔离 + 波次并发全自动；质量靠 implementer 报告前自跑门禁 + 合并门编译改动模块+fitness 兜底。
...
你是**编排器（Coordinator）**，不亲自写功能代码。你读任务计划，把每个任务派给一个**全新隔离上下文的 implementer 子代理**去实现，子代理回报 `done` 即完成。两大机制由你统一调度：
...
> **质量不靠独立验收子代理把关**（已去掉任务级独立验收反压），而是三层兜底：**implementer 报告前自跑门禁**（format/lint/typecheck + 一次性测试自验、验完即删，任一不过它自己不报 done）+ **合并门**（`/sdd:worktree finish` 在主终端只编译改动模块 + 架构 fitness）+ 功能级 **`/sdd:verify`** 行为验证。
...
- **未传参数 → 你按任务结构择优**：
  - 多波次、且波内有真并行（如 ≥2 个 Wave，或某波 ≥3 个 Boundary 不重叠可并行的任务），确定性调度 + Boundary/依赖闸/blocked 传播由代码保证能实质降低偏移/返工 → **选 Workflow 模式**。
...
把"波次并发 + Boundary/依赖闸/blocked 传播"从提示词软约束**硬化成代码控制流**：
1. **前置解析**：派一个解析子代理把 `tasks.md`（lite 则 `spec.md` 的 `## Tasks`）解析成结构化 `args = { featureDir, featureRoot, constitutionPath, waves:[{id,taskIds[]}], tasks:{Tn:{id,what,boundary[],depends[],refs,doneWhen,domain,verify,isolation}}, stackPacks, injectSkills }`。**`featureRoot` 传上面「工作根判定」算出的那个绝对路径**（hub 模式=该 feature 的 worktree，经典模式=当前目录）；脚本会把它注入每个 implementer 子代理 prompt，强制它们 `cd` 进去、只在此目录内操作。
2. **跑脚本（务必用内置脚本，别现场另写）**：用 **Workflow 工具**执行插件自带的 `workflows/sdd-implement.js`——`Workflow({ scriptPath: "<plugin>/workflows/sdd-implement.js", args })`。**任务/波次全部通过 `args` 传入，脚本本身不用改**（它已对任意 waves/tasks 通用、且经确定性校验合规）。脚本里：波内 `parallel` 并行 implementer、波间屏障、implementer `blocked`/越界的处理、Boundary 三道闸、依赖闸 blocked 传播——**全是代码保证，不靠 LLM 自觉**。
   - ⚠️ **不要现场即兴生成 workflow 脚本**——那极易踩 Workflow 的确定性校验（见下方"合规须知"）。内置脚本已覆盖通用情形；确需定制才改，且必须守合规须知。
3. **回填**：据脚本返回的结构化汇总，确定性回填 `tasks.md` 状态位（`[x]`/`[!]`）与 `Progress`；非空 `deviation` 回填 `design.md`(lite 则 spec.md) `## Deviations`；`needsHumanDecision`/blocked 原样转述给用户。收尾仍提示 `/sdd:verify` 做功能级行为验证（这是本流程唯一的独立验收环节）。
...
5. **回填状态**（每个回报的任务）：
   - **done**（implementer 已自跑门禁通过、守住 Boundary）→ **立即用 Edit 工具在 `tasks.md` 把该任务那行 `[ ]`/`[~]` 改成 `[x]`，并更新顶部 `Progress: X / N`**。⚠️ 这是**每个任务完成后当场必做的硬动作**，不是收尾再补——**别等全部做完才回填**（那样最容易漏）。回填完再继续下一个任务。
     - **抽查兜底（非强制验收，但别全盘轻信）**：implementer 自报门禁通过 = 采信，但你若发现回报里门禁结果缺失/可疑，可自行跑一次该任务门禁核对；真正的整体验收留给功能级 `/sdd:verify` 和合并门。
...
6. **波间纪律**：上一波没有全部完成（有 `blocked`），不开下一波依赖它的任务（失败不向下游扩散）。无依赖关系的其他分支可继续。

7. **完成后**：
   - **核对回填**：再扫一遍 `tasks.md`——每个已完成的任务都已是 `[x]`、`Progress` 数字与勾选数一致、阻塞的是 `[!]`。**有遗漏当场补勾**（防"代码做了、状态没更新"）。
   ...
   - 若全部完成 → 提示 `/sdd:verify` 做功能级行为验证（本流程唯一的独立验收环节，务必别跳）；仍有 `[!]` → 列出待人工决策项。
   - ⚠️ **本（feature）终端到此为止——不要在这里跑合并门**。各任务已跑过 §3 的任务级门禁（format/lint/typecheck + 一次性测试自验，尽量按改动范围）；**合并门（编译改动模块 + fitness）是 `/sdd:worktree finish` 在【主终端】的专属步骤**，在本终端跑只是把 finish 的活提前到错地方、白拖慢。收工后 Stop hook 若提醒"跑门禁"，跑的也是 scoped 的任务级自验，不是合并门。
