...
4. **一次性测试随实现走，一般不单独成任务**：本项目测试是一次性自验（验完即删），通常在实现任务里顺手写完就删，不必把 test 拆成独立 Boundary/任务。只有确要 TDD 先行时，才让"写测试"任务先分波。
   - ⚠️ **别铺一套一次性测试**（§4）：一次性测试写到够验证当前改动即可、验完即删；小改别为每条 AC 拆一个测试任务、别动辄起重容器集成测试。任务里写清"用几个聚焦用例/参数化验 AC，验过即删"，而不是"新建一堆 `XxxTest` 留库"。
