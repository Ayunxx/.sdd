...
- 实现阶段（`/sdd:implement`）以**双角色隔离编排器**运行，状态只能按 pending→implementing→implemented→verifying→verified→passed 推进。每个任务依次只派两个互不共享上下文的 fresh agent：Implementer 只修改实现/测试代码，不执行完成门禁；Verifier 只运行非写入 format-check/lint/typecheck/test 并逐条提交 Done when/AC evidence，核对前后工作树必须一致。Verifier 通过后即可标 `[x]`。**实现期间禁止 Implementer 或编排器为刚完成的单任务立即派 Reviewer**；所有任务完成后，`/sdd:verify` 才对完整 feature diff 统一派一次 fresh `code-reviewer`，再做功能级 AC 复验。
...
- **代码质量门禁**：Implementer 不给自己验收；由 fresh Verifier 跑 constitution §3 的非写入 format-check/lint/typecheck/test 并逐条证明 Done when/AC，回报实际命令、退出码、摘要和日志位置。任一非零、`not_run`、证据缺失或 Verifier 导致工作树变化都不算完成。禁止用 skip/only、恒真断言、过宽 mock、禁用类型/lint、降低覆盖率或删除回归来换绿灯；`/sdd:verify` 的唯一 feature Reviewer 会从整体 diff 再复核一次测试完整性。
